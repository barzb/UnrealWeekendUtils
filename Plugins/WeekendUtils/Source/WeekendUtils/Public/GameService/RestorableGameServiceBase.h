...
#include "AsyncGameServiceBase.h"
...
/**
 * Asynchronous game service base class that listens to restored SaveGames from @USaveGameService.
 * Offers virtual methods to implement for derived classes that respond to SaveGame events.
 * @note that dependencies to restorable services should call @WaitUntilServiceIsRunning().
 */
UCLASS(Abstract)
class WEEKENDUTILS_API URestorableGameServiceBase : public UAsyncGameServiceBase
{
	GENERATED_BODY()
